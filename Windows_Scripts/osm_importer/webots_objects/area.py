    ...
    def export(cls, file, noParkings):
        ...
        for area in Area.list[:]:
            ...
            if area.type == 'forest' and not Area.noForests:
                if file is not None:
                    # 1. Calculate the true center to use as a proper rotation pivot
                    xMin, xMax, yMin, yMax = OSMCoord.get_min_and_max_coord(area.ref)
                    anchor_x = (xMin + xMax) / 2.0
                    anchor_y = (yMin + yMax) / 2.0

                    file.write("Forest {\n")
                    file.write('  id "%s"\n' % area.OSMID)
                    
                    # 2. Place the origin at the exact center
                    file.write("  translation %.2f %.2f 0\n" % (anchor_x, anchor_y))
                    
                    # (Optional) Apply a default rotation here if your world requires it
                    # file.write("  rotation 0 0 1 0\n")
                    
                    file.write("  shape [\n")
                    
                    # 3. Write coordinates relative to the new center pivot
                    refs = area.ref if Area.are_references_clockwise(area.ref) else reversed(area.ref)
                    for ref in refs:
                        rel_x = OSMCoord.coordDictionnary[ref].x - anchor_x
                        rel_y = -(OSMCoord.coordDictionnary[ref].y - anchor_y)
                        file.write("    %.2f %.2f, " % (rel_x, rel_y))
                        
                    file.write("\n  ]\n")
                    file.write("  density %.3f\n" % area.density)
