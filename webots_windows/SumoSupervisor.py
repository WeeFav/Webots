...
class SumoSupervisor (Supervisor):
    ...
    def get_initial_vehicles(self):
        ...
        for i in range(0, self.vehiclesLimit):
            ...
            if node:
                ...
                self.vehicles[i].name.setSFString(defName)
                self.vehicleNumber += 1
                print(f"{defName} has been added. Route: {self.traci.vehicle.getRoute(id)}")
            else:
                break
        for i in range(0, self.vehiclesLimit):
    ...
    def get_vehicles_position(self, id, subscriptionResult, step, xOffset, yOffset,
        ...
        # shifts position backward by half the car length
        ...
        if index >= 0:
            ...
            # Handle height, pitch, roll for 3D roads in Webots
    ...
    def reassign_route(self, id):
        """Give a vehicle a brand-new destination before it finishes."""
        for _ in range(50):
            destination_edge = random.choice(self.all_edges)
            try:
                self.traci.vehicle.changeTarget(id, destination_edge)
                return True
    ...
    def run(self, port, disableTrafficLight, directory, step, rotateWheels,
        ...
        # parse the net and get the coordinate offsets
        # SUMO and Webots use different coordinate origins → this aligns them
        ...
        # Subscribe to new vehicles entering the simulation and how many vehicles remain
        ...
        while self.step(step) >= 0:
            ...
            # Get simulation results (new vehicles, remaining vehicles)
            ...
            for id in result[self.traci.constants.VAR_DEPARTED_VEHICLES_IDS]:
                if not id.startswith('webotsVehicle'):
                    self.traci.vehicle.subscribe(id, self.vehicleVariableList) # Subscribe to each new vehicle given the data to tack
            ...
            # For all vehicles in the simulation (newly added and existing)
            for id in idList:
            ...
            for id in idList:
                ...
                if route_index == len(route) - 1:
                    if self.reassign_route(id):
                        print(f"{self.vehicles[int(id)].name.getSFString()} has been re-routed. Route: {self.traci.vehicle.getRoute(id)}")
                    else:
                        print(f"Cannot re-route {self.vehicles[id].name.getSFString()} after 50 tries")
                    

            # SUMO simulation over (no more vehicle are expected or still driving)
